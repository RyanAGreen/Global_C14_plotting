...
fig,ax = plt.subplots(4,2,figsize=(8,10),sharex='col')
ax=ax.flatten()
...
ax[2].plot(both_1D[1].ADratio[0],D14Cerrorscore_two[1],color='#ff7f00',marker = 'o',markeredgecolor='k', linewidth=2, markersize=10)
ax[4].plot(both_1D[1].ADratio[0],totalerrorscore_two[1],color='#ff7f00',marker = 'o',markeredgecolor='k', linewidth=2, markersize=10)
ax[6].plot(both_1D[1].ADratio[0],Ccum1_two[1],marker = 'o',markeredgecolor='k',color='#ff7f00', linewidth=2, markersize=10)
# plot IS ISchange
...
ax[7].tick_params(labelbottom=True, labeltop=False, labelleft=False, labelright=True)
ax[6].tick_params(labelbottom=True, labeltop=False, labelleft=True, labelright=False)

for i in range(4):
    #ax[i].plot(twoD_totalratio,twoDlist[counter],color='#984ea3',marker='o',markeredgecolor='k', linewidth=2, markersize=10)
    ax[i*2].plot(CO2only_totalratio,CO2onlylist[i],color='#e41a1c',marker='o',markeredgecolor='k', linewidth=2, markersize=10)
    ax[i*2+1].plot(ISCO2only_totalratio,ISCO2onlylist[i],color='#e41a1c',marker='o',markeredgecolor='k', linewidth=2, markersize=10)
    # ax[i*2].axvspan(0, 0.5, alpha=0.25, color='green')
    # ax[i*2+1].axvspan(1.4, 1.6, alpha=0.25, color='green')
    #ax[i].boxplot(test,positions=[ISCO2onlylist[counter]], vert=False)
ax[1].set_ylim(55,145)
ax[3].set_ylim(70,130)
ax[5].set_ylim(60,140)
ax[7].set_ylim(0,3000)
ax[0].set_ylim(55,145)
ax[2].set_ylim(70,130)
ax[4].set_ylim(60,140)
ax[6].set_ylim(0,3000)

ax[6].set_xlim(-0.15,2.15)
ax[7].set_xlim(-0.15,2.15)


# get tick locations for No IS
locs0 = ax[0].get_yticks().tolist()
labels0 = ax[0].get_yticklabels()
locs1 = ax[2].get_yticks()
labels1 = ax[2].get_yticklabels()
locs2 = ax[4].get_yticks()
labels2 = ax[4].get_yticklabels()
locs3 = ax[6].get_yticks()
labels3 = ax[6].get_yticklabels()
...
bp=[0,1,1,1]
bp[0] = ax[0].boxplot(filtered_data,positions=[twoDlist[0]],widths=19,showfliers=True, vert=False, patch_artist=True,zorder=0)
bp[1] = ax[2].boxplot(filtered_data,positions=[twoDlist[1]],widths=12,showfliers=False, vert=False, patch_artist=True,zorder=0)
bp[2] = ax[4].boxplot(filtered_data,positions=[twoDlist[2]],widths=13,showfliers=False, vert=False, patch_artist=True,zorder=0)
bp[3] = ax[6].boxplot(filtered_data,positions=[twoDlist[3]],widths=500,showfliers=False, vert=False, patch_artist=True,zorder=0)
...
ax[2].set_yticks(ticks=locs1)
ax[2].set_yticklabels(labels=locs1)
ax[4].set_yticks(ticks=locs2)
ax[4].set_yticklabels(labels=locs2)
ax[6].set_yticks(ticks=locs3)
ax[6].set_yticklabels(labels=locs3)

# get tick locations for IS
locs0 = ax[1].get_yticks().tolist()
labels0 = ax[1].get_yticklabels()
locs1 = ax[3].get_yticks()
labels1 = ax[3].get_yticklabels()
locs2 = ax[5].get_yticks()
labels2 = ax[5].get_yticklabels()
locs3 = ax[7].get_yticks().tolist()
labels3 = ax[7].get_yticklabels()
...
bp1[0] = ax[1].boxplot(filtered_data,positions=[IStwoDlist[0]],widths=19,showfliers=True, vert=False, patch_artist=True,zorder=0)
bp1[1] = ax[3].boxplot(filtered_data,positions=[IStwoDlist[1]],widths=12,showfliers=False, vert=False, patch_artist=True,zorder=0)
bp1[2] = ax[5].boxplot(filtered_data,positions=[IStwoDlist[2]],widths=13,showfliers=False, vert=False, patch_artist=True,zorder=0)
bp1[3] = ax[7].boxplot(filtered_data,positions=[IStwoDlist[3]],widths=500,showfliers=False, vert=False, patch_artist=True,zorder=0)

for i in range(4):
    ## change outline color, fill color and linewidth of the boxes
    for box in bp1[i]['boxes']:
        # change outline color
        box.set( color='#7570b3', linewidth=2)
        # change fill color
        box.set( facecolor = '#984ea3' )
    ...
    ## change color and linewidth of the whiskers
    for whisker in bp1[i]['whiskers']:
        whisker.set(color='#7570b3', linewidth=2)

    ## change color and linewidth of the caps
    for cap in bp1[i]['caps']:
        cap.set(color='#7570b3', linewidth=2)

    ## change color and linewidth of the medians
    for median in bp1[i]['medians']:
        median.set(color='white', linewidth=3)

    ## change the style of fliers and their fill
    for flier in bp1[i]['fliers']:
        flier.set(marker='o', color='#e7298a', alpha=0.5)

# set ticks for IS
ax[1].set_yticks(ticks=locs0)
ax[1].set_yticklabels(labels=locs0)
ax[3].set_yticks(ticks=locs1)
ax[3].set_yticklabels(labels=locs1)
ax[5].set_yticks(ticks=locs2)
ax[5].set_yticklabels(labels=locs2)
ax[7].set_yticks(ticks=locs3)
ax[7].set_yticklabels(labels=locs3)

plt.rcParams["font.weight"] = "bold"
ax[0].set_ylabel('CO$_{2atm}$ score \n (%)',fontsize=12,fontweight='bold')
ax[2].set_ylabel('∆$^{14}$C$_{atm}$ score \n (%)',fontweight='bold',fontsize=12)
ax[4].set_ylabel('Total score \n (%)',fontweight='bold',fontsize=12)
ax[6].set_ylabel('Total carbon \n (PgC)',fontweight='bold',fontsize=12)
...
ax[5].annotate('', ha = 'center', va = 'bottom', xy = (1.7,118.5),xytext = (1.7, 102.5),arrowprops = {'facecolor' : 'red'},zorder=4)
ax[5].text(1.77, 110.5,'worse',fontsize='small')

ax[5].annotate('', ha = 'center', va = 'bottom', xy = (1.7,81.5),xytext = (1.7, 97.5),arrowprops = {'facecolor' : 'green'},zorder=4)
ax[5].text(1.77, 89.5,'better',fontsize='small')

ax[0].get_shared_y_axes().join(ax[0], ax[1])
ax[0].set_yticklabels([])
ax[2].get_shared_y_axes().join(ax[2], ax[3])
ax[2].set_yticklabels([])
ax[4].get_shared_y_axes().join(ax[4], ax[5])
...
ax[7].legend(handles=[oneDall_legend,CO21D_legend,both1D_legend,twoD_legend],ncol=2,frameon=False,bbox_to_anchor=(0.75, -0.25))

ax[6].set_xlim(-0.1,2.1)
