...
ISCO21D_optimal = ISCO21D[8]

# improvement of -7%, 0.0
# CO2_only = D14C1D[0]
# # improvement of 4%, 0.0
# ISCO2_only = ISCO21D[0]
# CO2 only performs much worse as D14C inversion (-99.3%) vs CO2 inversion (-32%)
# f.score(twoD_optimal,control)
# f.max(twoD_optimal)
# f.magnitude(ISD14C1D_optimal)[0]
#colors
...
D14C_atm = [D14C,control,CO2_only,CO21D_optimal,D14C1D_optimal,twoD_optimal]
IS_D14C_atm = [D14C,control,ISCO2_only,ISCO21D_optimal,ISD14C1D_optimal,IStwoD_optimal]

CO2_atm = [CO2obs,control,CO2_only,D14C1D_optimal,CO21D_optimal,twoD_optimal]
IS_CO2_atm = [CO2obs,control,ISCO2_only,ISD14C1D_optimal,ISCO21D_optimal,IStwoD_optimal]
...
ISlabels = ['Observation data','Control with changed IS','CO2 only -32% improvement','Optimal 1D CO$_2$ inversion 10.2% improvement','Optimal 1D  ∆$^{14}$C inversion 2.4% improvement','Optimal 2D inversion 12.8% improvement']
...
linestyles = ['-','--','-','-','-','-']
linewidths = ['2','2','2.5','2.5','2.5','2.5']
...
for i in range(6):
    ...
    ax[2].plot(CO2_atm[i].year,CO2_atm[i].CO2,linewidth=linewidths[i],ls=linestyles[i],color = colors[i],label= labels[i])
    # plot Crate for no IS change
    ax[4].plot(CO2_atm[i].year,CO2_atm[i].Crate,linewidth=linewidths[i],ls=linestyles[i],color = colors[i],label= labels[i])
    ax4.plot(CO2_atm[i].year,CO2_atm[i].Ccum,alpha=0.00001,linewidth=linewidths[i],ls=linestyles[i],color = colors[i],label= labels[i])
    ax4.hlines(y=CO2_atm[i].Ccum[200],linestyle='solid',lw=2.5,color=colors[i],xmin=-1, xmax=21)
    ...
    ax[3].plot(IS_CO2_atm[i].year,IS_CO2_atm[i].CO2,linewidth=linewidths[i],ls=linestyles[i],color = colors[i],label= ISlabels[i])
    # plot Crate for IS change
    ax[5].plot(IS_CO2_atm[i].year,IS_CO2_atm[i].Crate,linewidth=linewidths[i],ls=linestyles[i],color = colors[i],label= ISlabels[i])
    ax5.plot(IS_CO2_atm[i].year,IS_CO2_atm[i].Ccum, alpha=0.00001,linewidth=linewidths[i],ls=linestyles[i],color = colors[i],label= ISlabels[i])
    ax5.hlines(y=IS_CO2_atm[i].Ccum[200],linestyle='solid',lw=2.5,color=colors[i],xmin=-1, xmax=21)
...
for i in range(2):
    ...
    ax[i].text(15.9,515,'HS1',fontsize='x-small')
...
ax[2].set_ylabel('Atmospheric CO$_2$ (ppm)',fontweight='bold',fontsize=10)
ax[0].set_ylabel('Atmospheric ∆$^{14}$C (‰)',fontweight='bold',fontsize=10)
...
ax[4].set_ylabel('Rate of carbon addition (pgC/yr)',fontweight='bold',fontsize=10)
ax5.set_ylabel('Cumulative carbon added (pgC)',fontweight='bold',fontsize=10)
