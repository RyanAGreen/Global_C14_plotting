...
twoD_totalscore_smoothed,twoD_totalratio_smoothed = f.score(twoD_smoothed,control)
IStwoD_totalscore,IStwoD_totalratio = f.score(IStwoD_optimal,control)
IStwoD_totalscore_smoothed,IStwoD_totalratio_smoothed = f.score(IStwoD_smoothed,control)
...
twoD_D14Cscore_smoothed = f.D14Cscore(twoD_smoothed,control)
IStwoD_D14Cscore = f.D14Cscore(IStwoD_optimal,control)
IStwoD_D14Cscore_smoothed = f.D14Cscore(IStwoD_smoothed,control)
...
twoD_CO2score_smoothed = f.CO2score(twoD_smoothed,control)
IStwoD_CO2score = f.CO2score(IStwoD_optimal,control)
IStwoD_CO2score_smoothed = f.CO2score(IStwoD_smoothed,control)
CO2only_CO2score = f.CO2score(CO2_only,control)
ISCO2only_CO2score = f.CO2score(ISCO2_only,control)
# Cumlative carbon
twoD_Ccum = f.max(twoD_optimal)
twoD_Ccum_smoothed = f.max(twoD_smoothed)
IStwoD_Ccum= f.max(IStwoD_optimal)
IStwoD_Ccum_smoothed = f.max(IStwoD_smoothed)
CO2only_Ccum = f.max(CO2_only)
ISCO2only_Ccum= f.max(ISCO2_only)


#create list so I can plot in a row
twoDlist = [twoD_CO2score,twoD_D14Cscore,twoD_totalscore,twoD_Ccum,twoD_Ccum_smoothed]
twoDlist_smoothed = [twoD_CO2score_smoothed,twoD_D14Cscore_smoothed,twoD_totalscore_smoothed,twoD_Ccum_smoothed]
IStwoDlist = [IStwoD_CO2score,IStwoD_D14Cscore,IStwoD_totalscore,IStwoD_Ccum,IStwoD_Ccum_smoothed]
IStwoDlist_smoothed = [IStwoD_CO2score_smoothed,IStwoD_D14Cscore_smoothed,IStwoD_totalscore_smoothed,IStwoD_Ccum_smoothed]
...
for i in range(0,8,2):
    ...
    ax[i].plot(twoD_totalratio_smoothed,twoDlist_smoothed[counter],color='#f781bf',marker='o',markeredgecolor='k', linewidth=2, markersize=10)
...
for i in range(1,8,2):
    ...
    ax[i].plot(IStwoD_totalratio_smoothed,IStwoDlist_smoothed[counter],color='#f781bf',marker='o',markeredgecolor='k', linewidth=2, markersize=10)
