...
for i in range(len(CO2_atm)):
    ...
    ax[1].plot(CO2_atm[i].year,CO2_atm[i].CO2,linewidth=linewidths_fan[i],ls=linestyles_fan[i],color = colors_fan[i],alpha = alpha[i])
...
ax[2].bar(twoD_optimal.year, twoD_optimal.Crate,width=0.1, color = colorsAD,zorder=2)
ax[2].plot(twoD_optimal.year, twoD_optimal.Crate,color='#984ea3',lw=2.5, zorder=3)
ax[2].text(15.36, 0.2,"{:0.2f} PgC \n{:0.2f} ALK:DIC ratio".format(fir[0],fir[1]),fontweight='bold')
ax[2].text(7.2, 0.167,"{:0.2f} PgC \n{:0.2f} ALK:DIC ratio".format(sec[0],sec[1]),fontweight='bold')
ax[2].text(2.5, 0.1,"{:0.2f} PgC \n{:0.2f} ALK:DIC ratio".format(thi[0],thi[1]),fontweight='bold')
...
#
#plt.tight_layout()
plt.savefig('Figures/Figure2_noIS.pdf')
#plt.show()
